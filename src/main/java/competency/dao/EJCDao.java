...
public class EJCDao {
	...
	public List<Competency> getEmployeeJobCompetency(Competency c){
		return template.query("SELECT DISTINCT (tbl_job.job_title+' - '+tbl_departments.dept_name),"
				...
				+ "ELSE 'Desired level attained' END AS score FROM tbl_job INNER JOIN tbl_job_competency ON '3' = "
				+ "tbl_job_competency.job_competency_job_id INNER JOIN tbl_employees ON tbl_job.job_id = '3'  "
				+ "LEFT JOIN tbl_competency_status ON tbl_job_competency.job_competency_competency_id = "
				+ "tbl_competency_status.competency_status_competency_id and tbl_competency_status.competency_status_emp_id='"+c.getEmployee_id()+"'"
						+ "  inner JOIN tbl_competency ON tbl_job_competency.job_competency_competency_id = tbl_competency.competency_id "
						+ "INNER JOIN tbl_departments ON tbl_job.job_dept_id = tbl_departments.dept_id where tbl_employees.emp_id='"+c.getEmployee_id()+"'", new RowMapper<Competency>(){
			public Competency mapRow(ResultSet rs, int row)  throws SQLException{
